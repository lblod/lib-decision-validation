...
export async function shallash(publicationLink: string, proxyURL?: string): Promise<Bindings[]> {
  const bindingsStream: BindingsStream = await engine.queryBindings(
    `
...
where {
  ...
  `,
    {
      sources: [
        'http://centrale.vindplaats.lblod.info/sparql',
        'https://lblod.zottegem.be/LBLODWeb/Home/Overzicht/e3d9480acc0cf400e11ab7afe2bcea144038dd2ef71d77247d5159c7ce8efb1a/GetPublication?filename=Notulen_Gemeenteraad_22-01-2024.html',
      ],
      ...(proxyURL && { proxy: new ProxyHandlerStatic(proxyURL) }),
    },
  );
