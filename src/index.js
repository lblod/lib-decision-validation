...
exports.getBlueprintOfDocumentType = exports.validatePublication = exports.validateProperty = exports.fetchDocument = exports.getPublicationFromFileContent = exports.determineDocumentType = void 0;
...
function validateProperty(subject, propertyShape) {
    var result = {};
    propertyShape.forEach(function (p) {
        switch (p.get("p").value) {
            ...
            default: {
                console.log("default ".concat(p.get("p").value));
            }
        }
    });
    result.actualValue = subject
        .filter(function (s) { return s.get("p").value === result.path; })
        .map(function (s) { return s.get("o").value; });
...
*/
function validatePublication(publication, blueprint) {
    return __awaiter(this, void 0, void 0, function () {
        var subjectKeys, result;
        return __generator(this, function (_a) {
            subjectKeys = __spreadArray([], new Set(publication.map(function (p) { return p.get("s").value; })), true);
            result = [];
            // we should save the counts of properties per subject
            subjectKeys.forEach(function (subjectKey) {
                var _a, _b;
                var subject = publication.filter(function (p) { return p.get("s").value === subjectKey; });
                // console.log(`subject: ${subject}`)
                var subjectType = (_a = subject
                    .find(function (s) {
                    return s.get("p").value ===
                        "http://www.w3.org/1999/02/22-rdf-syntax-ns#type";
                })) === null || _a === void 0 ? void 0 : _a.get("o").value;
                var blueprintShapeKey = (_b = blueprint
                    .find(function (b) {
                    return b.get("p").value === "http://www.w3.org/ns/shacl#targetClass" &&
                        b.get("o").value === subjectType;
                })) === null || _b === void 0 ? void 0 : _b.get("s").value;
                var blueprintShape = blueprint.filter(function (b) { return b.get("s").value === blueprintShapeKey; });
                var propertyKeys = blueprintShape
                    .filter(function (b) { return b.get("p").value === "http://www.w3.org/ns/shacl#property"; })
                    .map(function (b) { return b.get("o").value; });
                if (subject.length > 0 && blueprintShape.length > 0) {
                    ...
                    propertyKeys.forEach(function (propertyKey) {
                        var propertyShape = blueprint.filter(function (b) { return b.get("s").value === propertyKey; });
                        resultSubject_1.validatedProperties.push(validateProperty(subject, propertyShape));
                        // console.log(`property: ${propertyShape}`)
                    });
                    result.push(resultSubject_1);
                }
            });
...
function getBlueprintOfDocumentType(documentType) {
    return __awaiter(this, void 0, void 0, function () {
        var blueprintLink, bindingsStream;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    blueprintLink = {
                        Notulen: "https://raw.githubusercontent.com/lblod/notulen-prepublish-service/master/test/shapes/meeting.ttl",
                        BesluitenLijst: "https://raw.githubusercontent.com/lblod/notulen-prepublish-service/master/test/shapes/decision-list.ttl",
...
exports.getBlueprintOfDocumentType = getBlueprintOfDocumentType;
